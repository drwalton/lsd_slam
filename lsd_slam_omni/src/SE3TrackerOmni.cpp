...
// tracks a frame.
// first_frame has depth, second_frame DOES NOT have depth.
SE3 SE3TrackerOmni::trackFrameOnPermaref(
		Frame* reference,
		Frame* frame,
		const SE3 &referenceToFrameOrg)
{
		...
		const SE3& frameToReference_initialEstimate)
{
	...
	// Iterate over pyramid levels used in hierarchical tracking.
	for(int lvl=SE3TRACKING_MAX_LEVEL-1;lvl >= SE3TRACKING_MIN_LEVEL;lvl--)
	{
		...
		//Select appropriate Levenberg-Marquardt lambda value for this level.
		float LM_lambda = settings.lambdaInitial[lvl];

		//Perform iterations of LM at this pyramid level.
...
float SE3TrackerOmni::calcWeightsAndResidual(
...
void SE3TrackerOmni::calcResidualAndBuffers_debugStart()
{
...
void SE3TrackerOmni::calcResidualAndBuffers_debugFinish(int w)
{
	if(plotTrackingIterationInfo)
	{
...
float SE3TrackerOmni::calcResidualAndBuffers(
...
void SE3TrackerOmni::calculateWarpUpdate(
		LGS6 &ls)
{
