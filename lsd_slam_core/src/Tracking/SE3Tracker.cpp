	...
	:model(model.clone())
{
	...
	buf_warped_residual = (float*)Eigen::internal::aligned_malloc(model.w*model.h*sizeof(float));
		...
		if(plotTrackingIterationInfo || plotResidual)
		{
			...
			if(plotTrackingIterationInfo)
			{
				setPixelInCvMat(&debugImageOldImageSource,getGrayCvPixel((float)resInterp[2]),
					static_cast<int>(u_new+0.5f),static_cast<int>(v_new+0.5f),(model->w/w));
				setPixelInCvMat(&debugImageOldImageWarped,getGrayCvPixel(
					(float)resInterp[2]),x,y,(model->w/w));
			}
			if(isGood)
				setPixelInCvMat(&debugImageResiduals,getGrayCvPixel(residual+128),x,y,(model->w/w));
			else
				setPixelInCvMat(&debugImageResiduals,cv::Vec3b(0,0,255),x,y,(model->w/w));
